...
def atr(candles):
    if len(candles)<ATR_LENGTH+1: return None
    trs=[]
    prev=None
    for c in candles:
        if prev is None: tr=c["high"]-c["low"]
        else: tr=max(c["high"]-c["low"],abs(c["high"]-prev),abs(c["low"]-prev))
...
def analyse(name,path):
    ...
    sessions={}
    for c in candles: sessions.setdefault(c["dt"].date(),[]).append(c)
    ...
    pdh=max((c["high"] for c in prev),default=None); pdl=min((c["low"] for c in prev),default=None)
    ...
    if now.time()<ELIGIBILITY:
        out["status"]="WAITING FOR 09:30"; return out
    if not today or pdh is None or pdl is None or a is None:
        out["status"]="DATA UNAVAILABLE"; out["status_class"]="red"; return out
    if not (pdl < op < pdh):
        out["status"]="NO TRADE — OPEN OUTSIDE RANGE"; out["status_class"]="red"; return out
    if len(today)<3:
        out["status"]="WAITING FOR 5M DATA"; return out
    # Only completed 5M candles are used; the last candle is considered live if its start is in the current 5M bucket.
    completed=[c for c in today if c["dt"].minute%5==0 and c["dt"].replace(second=0,microsecond=0)+timedelta(minutes=5)<=now]
    if len(completed)<2:
        out["status"]="WAITING FOR COMPLETED 5M CANDLE"; return out
    trigger=None; direction=None
    for c in completed:
        if c["close"]>=pdh+threshold: trigger=c; direction="LONG"; break
        if c["close"]<=pdl-threshold: trigger=c; direction="SHORT"; break
    if trigger is None:
        out["status"]="WAITING — NO DISPLACEMENT"; return out
    out["displacement"]="CONFIRMED"
    out["direction"]=direction
    idx=completed.index(trigger)
    if idx+1>=len(completed):
        out["status"]="DISPLACEMENT CONFIRMED — ACCEPTANCE WAITING"; return out
    confirm=completed[idx+1]
    accepted=(confirm["close"]>pdh if direction=="LONG" else confirm["close"]<pdl)
    if not accepted:
        out["status"]="NO TRADE — ACCEPTANCE FAILED"; out["status_class"]="red"; out["acceptance"]="FAILED"; return out
    out["acceptance"]="CONFIRMED"
    entry_bar=completed[idx+2] if idx+2<len(completed) else None
    if entry_bar is None:
        out["status"]="ACCEPTANCE CONFIRMED — ENTRY WAITING"; return out
...
def home():
    ...
    for name,path in (("NIFTY","/nifty-live"),("BANK NIFTY","/banknifty-live")):
        try: items.append(analyse(name,path))
        except Exception as e: items.append({"name":name,"generated":datetime.now(IST).strftime("%d %b %Y %H:%M:%S IST"),"pdh":"—","pdl":"—","open":"—","atr":"—","threshold":"—","long_trigger":"—","short_trigger":"—","status":"DATA ERROR: "+str(e),"status_class":"red","displacement":"—","acceptance":"—","direction":"—","entry":"—","sl":"—","tp":"—","bars":"—"})
    return render_template_string(HTML,instruments=items)

@app.route("/health")
def health(): return jsonify({"status":"ok","service":"pdra-monitor"})
