...
<style>
body{font-family:Arial;background:#080b12;color:#eef2f7;padding:18px}.wrap{max-width:1400px;margin:auto}
...
.row{display:flex;justify-content:space-between;padding:7px 0;border-bottom:1px solid #242c38;gap:12px}
...
.badge{display:inline-block;padding:5px 9px;border-radius:999px;background:#182130;margin:3px 4px 3px 0;font-size:12px}
table{width:100%;border-collapse:collapse;font-size:12px}td,th{padding:8px;border-bottom:1px solid #29313d;text-align:left}
</style>
<div class="wrap">
<div class="banner"><h1>PSYCHO INTRADAY OPPORTUNITY ENGINE</h1><h3>{{session.status}}</h3><div class="muted">{{session.detail}} • {{scan_time or 'waiting'}} • 15s scan • INTRADAY ONLY</div></div>
<div class="grid">{%for x in items%}<div class="card">
<h2>{{x.name}}</h2><h3 class="{{x.cls}}">{{x.status}}</h3>
<div class="row"><span>State</span><b>{{x.state}}</b></div><div class="row"><span>Direction</span><b>{{x.direction}}</b></div><div class="row"><span>Setup</span><b>{{x.setup}}</b></div><div class="row"><span>Signal</span><b>{{x.signal_id}}</b></div><div class="row"><span>Contract</span><b>{{x.contract}}</b></div>
<div class="row"><span>Entry / Actual LTP</span><b>{{x.entry}}</b></div><div class="row"><span>Bid / Ask</span><b>{{x.bid}} / {{x.ask}}</b></div><div class="row"><span>SL</span><b>{{x.sl}}</b></div><div class="row"><span>TP</span><b>{{x.tp}}</b></div><div class="row"><span>Live premium</span><b>{{x.live}}</b></div>
<div class="row"><span>Score</span><b>{{x.score}}/100</b></div><div class="row"><span>Time left</span><b>{{x.time_left}}</b></div><div class="row"><span>TP distance</span><b>{{x.tp_distance}}</b></div><div class="row"><span>TP feasibility</span><b>{{x.tp_feasibility}}</b></div><div class="row"><span>Intraday deadline</span><b>15:40 IST</b></div>
<div class="reason"><b>WHY</b><br>{{x.reason}}</div></div>{%endfor%}</div>
<div class="trades"><h2>TODAY'S CLOSED TRADES</h2>{%if trades%}<table><tr><th>Exit</th><th>Instrument</th><th>Signal</th><th>Setup</th><th>Contract</th><th>Entry</th><th>Exit</th><th>Result</th></tr>{%for t in trades%}<tr><td>{{t.exit_time}}</td><td>{{t.name}}</td><td>{{t.signal_id}}</td><td>{{t.setup}}</td><td>{{t.contract}}</td><td>{{t.entry}}</td><td>{{t.exit}}</td><td class="{{'good' if t.result=='TP TAKEN' else 'bad'}}">{{t.result}}</td></tr>{%endfor%}</table>{%else%}<div class="muted">No completed trades today.</div>{%endif%}</div>
<div class="hunts"><h2>TODAY'S HUNT / MISSED OPPORTUNITIES</h2>{%if hunts%}<table><tr><th>Time</th><th>Instrument</th><th>Setup</th><th>Direction</th><th>Score</th><th>Outcome</th></tr>{%for h in hunts[-20:]%}<tr><td>{{h.time}}</td><td>{{h.name}}</td><td>{{h.setup}}</td><td>{{h.direction}}</td><td>{{h.score}}</td><td>{{h.outcome}}</td></tr>{%endfor%}</table>{%else%}<div class="muted">No rejected or missed hunts recorded today.</div>{%endif%}</div>
</div>'''
...
def get(path):
    req=urllib.request.Request(BR+path,headers={"Accept":"application/json","User-Agent":"PSYCHO-IOE/6.0"})
...
def atr(c,n=14):
    ...
    tr=[];pc=None
    for x in c:
        tr.append(x["high"]-x["low"] if pc is None else max(x["high"]-x["low"],abs(x["high"]-pc),abs(x["low"]-pc)));pc=x["close"]
    return sum(tr[-n:])/n
def ema(v,n=20):
...
def vwap(c):
    pv=vol=0
    for x in c:q=x["volume"];pv+=((x["high"]+x["low"]+x["close"])/3)*q;vol+=q
    return pv/vol if vol else None
def rng(x):return max(x["high"]-x["low"],.00001)
...
def option_rows(raw):
    ...
    n=raw.get("option_chain") or raw.get("data") or raw; strikes=n.get("strikes") if isinstance(n,dict) else None
...
def bundle(name):
    slug="nifty" if name=="NIFTY" else "banknifty";return get("/"+slug+"-live"),get("/"+slug+"-option-chain")
def pick(raw,underlying,direction):
    wanted="CE" if direction=="LONG" else "PE";ranked=[]
    for x in [z for z in option_rows(raw) if z["side"]==wanted]:
        score=0;delta=abs(x["delta"] or 0);dist=abs(x["strike"]-underlying)
        score+=25 if dist<=max(50,underlying*.0025) else 12 if dist<=max(100,underlying*.005) else 0
        score+=25 if .40<=delta<=.65 else 15 if .30<=delta<=.75 else 0
        ltp,bid,ask=x["ltp"],x["bid"],x["ask"]
        if bid is not None and ask is not None and ask>=bid>0:
            spread=(ask-bid)/max(ltp,.01);gap=abs(ask-ltp)/max(ltp,.01)
            if spread<=.005 and gap<=.008:score+=30
            elif spread<=.01 and gap<=.015:score+=20
            elif spread<=.02 and gap<=.025:score+=8
            else:score-=35
        else:score-=20
        score+=10 if (x["oi"] or 0)>0 else 0;score+=10 if (x["volume"] or 0)>0 else 0
...
def detect(name,raw,t):
    c=tf(raw,"1M") or candles(raw);c=[x for x in c if x["dt"]+timedelta(minutes=1)<=t];c5,c15,c1h=tf(raw,"5M"),tf(raw,"15M"),tf(raw,"1H")
    if len(c)<25:return None
    u,a=c[-1]["close"],atr(c)
    if a is None:return None
    pd=prev(c,t);orh,orl=opening(c,t);day=[x for x in c if x["dt"].date()==t.date()];vw=vwap(day);e5=ema([x["close"] for x in c5],20);e15=ema([x["close"] for x in c15],20);e1h=ema([x["close"] for x in c1h],20)
    ...
    if e5 and e15:
        ...
        if u<e5 and u<e15 and x2["high"]>=e5*.999 and bear(x1) and x1["close"]<x2["low"]:candidates.append((83,"SHORT","PULLBACK CONTINUATION",["5M/15M trend aligned","pullback into mean","bearish rejection/continuation"]))
    ...
    mtf=sum(v is not None and ((u>v) if direction=="LONG" else (u<v)) for v in (e5,e15,e1h));vw_ok=vw is not None and ((u>vw) if direction=="LONG" else (u<vw));score=min(100,base+mtf*4+(4 if vw_ok else 0));why += [f"MTF alignment {mtf}/3","VWAP aligned" if vw_ok else "VWAP not aligned"]
...
def blank(name,status="SCANNING",reason="No qualified setup."):
    return {"name":name,"signal_id":"—","state":"SCANNING","status":status,"cls":"blue","direction":"—","setup":"NONE","contract":"—","entry":"—","bid":"—","ask":"—","sl":"—","tp":"—","live":"—","score":0,"reason":reason,"time_left":"—","tp_distance":"—","tp_feasibility":"—"}

def minutes_left(t):
    close_dt=datetime.combine(t.date(),CLOSE,tzinfo=IST);return max(0,(close_dt-t).total_seconds()/60)

def feasibility(entry,tp,live,atr_opt,t):
    left=minutes_left(t);dist=max(0,tp-live);atrv=max(atr_opt or 0.0,.01)
    # Conservative option-premium travel estimate: recent ATR proxy plus time decay in available minutes.
    required_units=dist/atrv
    expected_units=max(.15,(left/5.0)*.35)
    ratio=expected_units/required_units if required_units>0 else 9
    if left<=5:return "LOW",ratio
    if ratio>=1.35:return "HIGH",ratio
    if ratio>=.75:return "MEDIUM",ratio
    return "LOW",ratio

def time_fields(entry,tp,live,t,under_atr):
    left=minutes_left(t);dist=max(0,tp-live); # translate underlying ATR to a conservative option movement proxy
    opt_atr=max(entry*.035,under_atr*.35)
    grade,ratio=feasibility(entry,tp,live,opt_atr,t)
    return fmt(left)+" min",fmt(dist),grade,ratio

def build_signal(name,direction,opt,quality,t,det):
    entry=opt["ltp"];risk=max(entry*.10,det["atr"]*max(abs(opt.get("delta") or .5),.35)*.20);sl=round(max(.05,entry-risk),2);tp=round(entry+2*(entry-sl),2)
    sid=f"IOE-{name.replace(' ','')}-{t:%Y%m%d-%H%M%S}"
    tl,td,fg,fr=time_fields(entry,tp,entry,t,det["atr"])
    return {"name":name,"signal_id":sid,"state":"ACTIVE","status":"🟢 SIGNAL ACTIVE — LOCKED","cls":"good","direction":direction,"setup":det["setup"],"contract":f"{int(opt['strike']):,} {opt['side']}","strike":opt["strike"],"side":opt["side"],"entry":fmt(entry),"entry_raw":entry,"bid":fmt(opt["bid"]),"ask":fmt(opt["ask"]),"sl":fmt(sl),"sl_raw":sl,"tp":fmt(tp),"tp_raw":tp,"live":fmt(entry),"score":det["score"],"reason":" • ".join(det["why"]+[f"option quality {quality}/100","ENTRY = CURRENT OPTION LTP","actual quote verified before activation","option locked; no reselection"]),"created":t.isoformat(),"time_left":tl,"tp_distance":td,"tp_feasibility":fg,"tp_ratio":fr}
...
def monitor(name,t):
    ...
    left=minutes_left(t);dist=max(0,s["tp_raw"]-live);opt_atr=max(s["entry_raw"]*.035,.01);fg,ratio=feasibility(s["entry_raw"],s["tp_raw"],live,opt_atr,t)
    s["time_left"]=f"{left:.1f} min";s["tp_distance"]=fmt(dist);s["tp_feasibility"]=fg;s["tp_ratio"]=ratio
    if fg=="LOW" and left<=20 and live<s["tp_raw"]:s["status"]="🟠 ACTIVE — TP TIME FEASIBILITY LOW"
    else:s["status"]="🟢 SIGNAL ACTIVE — LOCKED"
    result=None
    if live>=s["tp_raw"]:result="TP TAKEN"
    elif live<=s["sl_raw"]:result="SL TRIGGERED"
    elif t.time()>=CLOSE:result="SESSION CLOSED"
    if result:
        trade={"exit_time":t.strftime("%H:%M:%S IST"),"name":name,"signal_id":s["signal_id"],"setup":s["setup"],"contract":s["contract"],"entry":s["entry"],"exit":fmt(live),"result":result,"tp_feasibility_at_exit":fg,"time_left_at_exit":left}
...
def session_info(t):
    ...
    return {"status":"MARKET LIVE","detail":"15-second scan • intraday only • hard session exit 15:40 IST."}
...
@app.route("/health")
def health():return jsonify({"status":"ok","service":"psycho-opportunity-engine-v6","last_scan":STATE["scan_time"]})
@app.route("/api/state")
